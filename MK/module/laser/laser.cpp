...
void laser_fire(int intensity = 100.0){
	...
	if (intensity > 10000.0) intensity = 10000.0; // restrict intensity between 0 and 10000 - These changes by Downunder35m allow for higher power resolution required for the raster engraving
	if (intensity < 0) intensity = 0;

    pinMode(LASER_FIRING_PIN, OUTPUT);
	#if LASER_CONTROL == 1
	  analogWrite(LASER_FIRING_PIN, labs((intensity / 10000.0)*(F_CPU / LASER_PWM)));
    #endif
	#if LASER_CONTROL == 2
      analogWrite(LASER_INTENSITY_PIN, labs((intensity / 10000.0)*(F_CPU / LASER_PWM)));
